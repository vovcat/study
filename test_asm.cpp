...
// === asm_start=0x4e4100 asm_end=0x4e5dc9 size=7369
// === smsw=80050033 80050033 PE MP ET NE WP AM PG
// === cs=ffff0073
// g++ -g -O0 -Wall -Wextra -fno-exceptions -fno-rtti test_asm.cpp -o test_asm && qemu-i386 ./test_asm
// === asm_start=0x40003100 asm_end=0x40004dc9 size=7369
// === smsw=00000001 00000001 PE
// === cs=ffff0023
// gcc -m64 -Wa,--defsym,__x86_64__=1 -mcmodel=large -fno-pic -fno-pie -g -O0 -Wall -Wextra test_asm.cpp -o test_asm && qemu-x86_64 ./test_asm
// === asm_start=0x40202100 asm_end=0x4020411b size=8219
// === smsw=80010001 80010001 PE WP PG
// === cs=ffff0033
// gcc -m64 -Wa,--defsym,__x86_64__=1 -mcmodel=large -g -O0 -Wall -Wextra test_asm.cpp -o test_asm && qemu-x86_64 ./test_asm
// === asm_start=0x40202100 asm_end=0x1e801e001d801d0 size=-1044913968
// === smsw=80010001 80010001 PE WP PG
// === cs=ffff0033
// (gcc -m64 -Wa,--defsym,__x86_64__=1 -mcmodel=large -Wl,-M -g -O0 -Wall -Wextra test_asm.cpp -o test_asm && qemu-x86_64 ./test_asm) 2>&1 |v
// gcc -m64 -Wa,--defsym,__x86_64__=1 -g -O0 -Wall -Wextra test_asm.cpp -o test_asm && qemu-x86_64 ./test_asm
// === asm_start=0x40202100 asm_end=0x40204147 size=8263
// === smsw=80010001 80010001 PE WP PG
// === cs=ffff0033
...
.macro .str reg:req, string:req
        .ifndef __x86_64__
        mov e\reg, offset str\@
        .else
        movabs r\reg, offset str\@
        .endif
        .pushsection .data1.str, "awS", @progbits
        .global str\@
        .type str\@, @object
        str\@: .string "\string"
        .size str\@, .-str\@
...
t2:	.ascii "ascii here\n"
.obs t3; .asciz "asciz here\n"; .obe t3
t_:
        .balign 16
.obs b1; .byte 123; .obe b1
b_:
        .balign 2
w1:	.short	234
w2:     .word	235
...
.fns func1	# int func1(char c, int idx) { int a, b; ....; return ax; }
        .ifndef __x86_64__
        # [esp] -> [ebp-8]    | b         |
        #          [ebp-4]    | a         |
        # [ebp] ->            | old_ebp   |
...
.fns prefetch_len
        .ifndef __x86_64__
            mov edi, offset 1f
        .else
            movabs rdi, offset 1f
        .endif
        #mov al, 0x90		# b0 90
        mov eax, 0x90909090	# b8 90 90 90 90
        mov ecx, (2f - 1f) / 4	# b9 10 00 00 00
        xor ebx, ebx	# 31 db
        inc ebx		# ff c3 (mov ebx, 1 # bb 01 00 00 00)
        .balign 8
        xor edx, edx	# 31 d2
        xor edx, edx	# 31 d2
        xor edx, edx	# 31 d2
        rep stosd	# f3 ab (es:[{e,r}di], eax)
1:	.rept 128
        add edx, ebx	# 01 da
        .endr
2:      mov eax, edx
...
.ifdef __x86_64__
	add BYTE PTR [rax],al		# 00 00
	add rax, rcx			# 48 01 c8
	add rax, 0x12			# 48 83 c0 12
	add rax, 0x1234			# 48 05 34 12 00 00
...
add_test1:
        .code64
	mov rax, rbx			# 48 89 d8             = dec eax; mov eax,ebx
	mov rax, [rbx-1]		# 48 8b 43 ff          = dec eax; mov eax, [ebx-0x1]
	mov rax, [rip-1]		# 48 8b 05 ff ff ff ff = dec eax; mov eax, [0xffffffff]
	mov byte ptr [rip-1], 0x90	# c6 05 ff ff ff ff 90 = mov byte ptr [0xffffffff], 0x90
        .code32
...
.fns smc_test1
	mov eax, 0x90	# b8 90 00 00 00
	call 1f		# e8 00 00 00 00
1:	pop edi		# 5f
	add edi, 5	# 83 c7 05
	stosb		# aa
	inc edi		# 47
	inc eax		# 40
	dec eax		# 48
	sub eax, 0x90	# 2d 90 00 00 00
	mov byte ptr [edi], 0x90 # c6 07 90
	mov al, 0x90	# b0 90
.fne smc_test1
...
.endif
        .set $mw.x, 0x2345
        add ax, $mw.x				 # 66 05 34 12

        $mw.y = 0x1234
        add ax, $mw.y				 # 66 05 34 12
        add eax, 0x12345678			 # 05 78 56 34 12
...
.ifndef __x86_64__
	mov eax, offset abs1
        ...
        mov eax, offset abs3
.else
	movabs rax, offset abs1
        movabs rax, offset abs2
        movabs rax, offset abs3
.endif
.fne absfn
...
abs3:	.space 10
        ...
        .type asm_init_ptr, @object
        .size asm_init_ptr, 8
asm_init_ptr:
.ifndef __x86_64__
        .long offset asm_init
.else
        .quad offset asm_init
.endif
...
{
    unsigned r;
    asm ("xor %%eax, %%eax; dec %%eax; mov %%cs, %%ax; mov %%eax, %0" : "=a" (r) :: /*clobber*/ "cc");
    return r;
}

unsigned fninit_smsw()
{
    unsigned r;
    asm ("xor %0, %0; fninit; fld st(0); smsw %0" : "=r" (r));
    return r;
}
...
enum CR0 : unsigned {
    ...
    CD = (1u << 30),
    PG = (1u << 31),
    ALL = PE | MP | EM | TS | ET | NE | WP | AM | NW | CD | PG
};
...
const char *desc_cr0(unsigned cr0)
{
    static char buf[128];
    sprintf(buf, "%08x%s%s%s%s%s%s%s%s%s%s%s", cr0,// & ~CR0::ALL,
        (cr0 & CR0::PE) ? " PE" : "",
        (cr0 & CR0::MP) ? " MP" : "",
        (cr0 & CR0::EM) ? " EM" : "",
        (cr0 & CR0::TS) ? " TS" : "",
...
extern "C" int smc_test(void);

extern char asm_start[], asm_end[];

extern char __executable_start[], _init[], _start[], __libc_csu_init[], _fini[],  _etext[];
extern char __rela_iplt_start[], __rela_iplt_end[];
...
extern char _edata[], __bss_start[], _end[];

extern "C" void abs1();
extern char abs2[];
extern const char* abs3;
...
void cat_proc_maps()
{
    ...
    FILE *fp = fopen(buf, "r");
    while (size_t sz = fread(buf, 1, sizeof(buf), fp))
...
void (*test_preinit_ptr)(void) __attribute__ ((section(".preinit_array"))) = test_preinit;
void (*test_init_ptr)(void) __attribute__ ((section(".init_array"))) = test_init;
void (*test_fini_ptr)(void) __attribute__ ((section(".fini_array"))) = test_fini;

int main()
{
    printf("=== __executable_start=%p " //"__rela_iplt_start=%p __rela_iplt_end=%p\n\t"
        "_init=%p _start=%p __libc_csu_init=%p\n\t" "_fini=%p _etext=%p\n\t"
        "__preinit_array_start=%p __preinit_array_end=%p\n\t"
        "__init_array_start=%p __init_array_end=%p\n\t"
        "__fini_array_start=%p __fini_array_end=%p\n\t"
        "_edata=%p __bss_start=%p _end=%p\n\t"
        "main=%p absfn=%p asm_start=%p asm_end=%p\n",
        __executable_start, //__rela_iplt_start, __rela_iplt_end,
        _init, _start, __libc_csu_init, _fini, _etext,
        __preinit_array_start, __preinit_array_end,
        __init_array_start, __init_array_end,
        __fini_array_start, __fini_array_end,
        _edata, __bss_start, _end,
        main, absfn, asm_start, asm_end);
